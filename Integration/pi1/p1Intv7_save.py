...
# CAN Setup
# -----------------------------------------
bus = can.Bus(channel='can0', interface='socketcan', bitrate=500000)		 #Initialize CAN Bus with 500Kbps bitrate
...
def send_can_message(can_id, data):						 #Function sends hex data over CAN given 11-bit hex ID and hex data
    ...
    while not success:								 #Keep trying to send until successful
        try:
            ...
            print("trying again")							 #If there is an error sending...
...
GPIO.add_event_detect(17, GPIO.RISING, callback=pulse_callback1, bouncetime=1)	 #Set Hall1 event
#Hall Effect 2
GPIO.setup(27, GPIO.IN, pull_up_down=GPIO.PUD_OFF)				 #Set GPIO of Hall2
GPIO.add_event_detect(27, GPIO.RISING, callback=pulse_callback2, bouncetime=1)	 #Set Hall2 event
#Start Switch
...
# Sampling Intervals
# -----------------------------------------
pot_rate = 600									 #This is the frequency capture in Hz for potentiometers
