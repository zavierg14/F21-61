...
def setConfig(device):
    ...
    device.writeReg(0x03, 6)       # 10HZ    Set the transmission back rate to 10HZ
    time.sleep(0.1)                # Sleep 100ms
...
def FiledCalibration(device):
    ...
    if input("Please rotate the three axes slowly around the XYZ axis, and then the calibration is completed （Y/N)？").lower()=="y":
